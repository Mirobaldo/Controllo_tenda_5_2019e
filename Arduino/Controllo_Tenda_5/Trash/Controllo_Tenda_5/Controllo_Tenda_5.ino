...
// VERSION display: Vx.amggn x=versione a=anno+2019 m=mese(hex, da 1 a C) gg=giorno - git commit name: -m "V5.191126.B <text>"
#define VERSION_ "V5.191126.C" // Test con Arduino
//efine VERSION_ "V5.191126.B" // Test con Arduino
//efine VERSION_ "V5.191126.A" // Test con Arduino
//efine VERSION_ "V5.191103.J" // SimpleDHT con DHT22 nuovo correzioni
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  if (RoutineInLoop50ms) // && (arrayStart==0) && (SUNBStart==false))
  {
    ...
    if(result>0) 
    {
      ...
      Serial.print(F("> ")); Serial.println(sCommand);
      inString[0]='\0';  
...
// -------------------------------------------------------------------------------------------------
  ...
  //  ResetRequest++;
    ...
    else if (CntSwOpen > SWITCHTIMEFILTER)           // Se open tenda
    {
      if (smWindAlarm == WA_NOALARM_00)              // Se non attivo allarme vento
      {
  //    Serial.print(F("+ SUNBstep up find:")); Serial.println(SUNBstep);
        ...
        else if (SUNBstep==0)                        // Solo se non attivo SUNBstep
        {
          if(SUNBpos<16) SUNBstep = 4-(SUNBpos%4);     // STEP up
           Serial.print(F("+ SUNBstep up set:")); Serial.println(SUNBstep); // -> Commenta 
        }
      }
      else
      {
    ...
    else if (CntSwDown > SWITCHTIMEFILTER)           // Se down tenda
    {
      if (smWindAlarm == WA_NOALARM_00)              // Se non attivo allarme vento
      {
//    Serial.print(F("+ SUNBstep dw find:")); Serial.println(SUNBstep);
        ...
        else if (SUNBstep==0)                        // Solo se non attivo SUNBstep
        {
          ...
          Serial.print(F("+ SUNBstep dw set:")); Serial.println(SUNBstep); // -> Commenta
        }
      }
      else
      {
...
void ReadTempHigro(void)
{
  ...
  if(timing==0)
  {
  	...
  	Serial.print((float)Temperature); Serial.print(F(" *C, "));
...
{
//Serial.print(F("version: ")); Serial.println(version_);
  strncpy(graphicversion,version_+1,2);
  graphicversion[2]='\0';
//Serial.print(F("graphicversion: ")); Serial.println(graphicversion);
  char s1[8];
// Conversione anno
...
//Serial.print(F("s1 anno: ")); Serial.println(s1);
  strcat(graphicversion,s1);
//Serial.print(F("graphicversion: ")); Serial.println(graphicversion);
// Conversione mese 
  strncpy(s1,version_+5,2);
  s1[2]='\0';
//Serial.print(F("s1 mese: ")); Serial.println(s1);
  n=atoi(s1);
//Serial.print(F("n mese: ")); Serial.println(n);
  if(n<10) s1[0]=char(n+int('0')); 
  else     s1[0]=char(n-10+int('A')); 
  s1[1]='\0';
//Serial.print(F("s1 mese2: ")); Serial.println(s1);
  strcat(graphicversion,s1);
//Serial.print(F("s1 mese: ")); Serial.println(s1);
// Conversione giorno 
