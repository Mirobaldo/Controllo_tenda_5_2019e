...
//--------------------------------------------------------------------------------------------------
void MatrixDisplay::MatrixWriteBar(int8_t data[], int pos, char barset)
{
...
//--------------------------------------------------------------------------------------------------
void MatrixDisplay::CleanBarDisplay(void)                  // Clean display
{                                        //
  byte     data_byte, addr;              // *
  uint32_t data_row;                     // *
                                         // *
...
void MatrixDisplay::RotateCH(int codeASCII, byte *RotCH) // Debug completato, OK
{
  ...
  RotCH[0]=pgm_read_byte_near(CH[codeChar][0]);

  for(ptrCH=1; ptrCH<(pgm_read_byte_near(CH[codeChar][0]+1)); ptrCH++)
  {
    rowCHdata = pgm_read_byte_near(CH[codeChar][ptrCH]);
